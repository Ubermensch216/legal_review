...
### 실측: 단계형 사례 01 (2026-09-23, 수정 전 코드)

같은 사례 01(취업규칙 HWPX)을 단계형으로 돌린 결과. 측정 중 사용자 개발 서버의 검토 요청이 한 건 끼어들어 수정 조문 호출이 약 7분 대기했으므로, 시간은 벽시계가 아니라 모델 시간(prefill+디코드)으로 비교한다.

| 항목 | 단일 호출 | 단계형 |
|---|---:|---:|
| 모델 호출 | 1 | 9 (S1, S3×2, S4×4, S5, 수정 조문 1) |
| 최대 입력 | 23,132토큰 | 12,722토큰 (S1은 7,286) |
| 디코드 속도 | 8.3 tok/s | 13.5~16.6 tok/s (컨텍스트가 작아서) |
| 출력 합계 | 4,046토큰 | 8,908토큰 |
| 모델 시간 | 736초 | 1,052초 |
| 결과 | 완료(부분) | 완료(부분), 쟁점 5개·수정 조문 1건 |
| 인용 조문 | 5개(정답 적중 4) | 6개(정답 7개 중 6개 인용) |

관찰과 조치:

1. **접두부 캐시는 실제 모델에서 작동한다.** S3는 S1의 공통 접두부 6,846토큰을 재사용했다(새로 처리 1,588토큰). 다만 gemma의 슬라이딩 윈도 주의 때문에 Ollama는 저장된 체크포인트 위치까지만 복원하므로, S4는 12.7k 중 5.2k만 캐시에서 나왔다.
