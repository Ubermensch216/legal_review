...
- **단위 테스트 (`npm test`)**: **273건 전수 통과, 0건 실패** (외부 통신 격리 픽스처 기반 회귀 검증).
...
- 외부 전문가 질의서의 단계형 검토 입력이 로컬 AI 예산을 넘으면 질문별 관련 쟁점·사실만 추려 예산에 맞는 묶음으로 추상화한다. 저장된 계약 검토 이력의 질문 9건을 실제 로컬 모델로 재현해 질의 9건과 공백 연결 9건을 모두 보존했다(질의서 저장은 메모리 DB에서 검증).
