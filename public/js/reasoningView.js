...
import { CONTRACT_LEGAL_EXPLANATION, explainConclusionReasons, explainContractRisk, explainExternalFacts, isSourcePlaceholder } from './conclusionLanguage.js';
...
function issueConclusion(issue, finding, readable) {
  const c = issue.conclusion || {};
  const reasons = explainConclusionReasons(c, issue.elements || [], readable);
  if (!finding) return { summary: (LEGAL[c.legal] || LEGAL.CONDITIONAL)[0], reasons };
  const status = finding.legalValidityConclusion?.status || finding.legalValidity;
  return {
    summary: explainContractRisk(finding),
    legal: CONTRACT_LEGAL_EXPLANATION[status] || '법적 판단 상태를 확인해야 합니다.',
    reasons,
    status
  };
}
...
function conclusionBody(issue, contract, label, tone, readable) {
  const c = issue.conclusion || {};
  const result = issueConclusion(issue, contract[0], readable);
  const reasonList = result.reasons.length
    ? `<div class="rv-conclusion-reasons"><b>${c.legal === 'CONDITIONAL' || ['AUTHORITY_INCOMPLETE', 'CONDITIONAL_ON_MATERIAL_FACT', 'FAILED'].includes(result.status) ? '아직 결론을 내리지 못한 이유' : '판단의 제한 사항'}</b>${list(result.reasons.map(esc), '')}</div>`
    : '';
  if (!contract.length) return `<div class="rv-conclusion"><strong class="rv-verdict rv-${tone}">${esc(label)}</strong><span>${esc(PROOF[c.proof] || '입증 미평가')}</span></div>
    <p class="rv-conclusion-summary">${esc(result.summary)}</p>${reasonList}
    ${c.ifResolved ? `<p class="rv-note">먼저 판단해야 할 쟁점이 해결되면: ${esc((LEGAL[c.ifResolved] || LEGAL.CONDITIONAL)[0])}</p>` : ''}
    ${issue.stageStatus === 'FAILED' ? '<p class="rv-note">판단 과정이 완료되지 않아 결론을 확인해야 합니다.</p>' : ''}`;
  const externalFacts = explainExternalFacts(contract.flatMap(f => f.additionalFactDetails || []));
  return `<div class="rv-conclusion"><strong class="rv-verdict rv-${tone}">${esc(label)}</strong></div>
    <p class="rv-conclusion-summary">${esc(result.summary)}</p>
    <p><b>법적으로 어디까지 판단했나</b> ${esc(result.legal)}</p>
    ${reasonList}
    <p><b>추가로 확인할 사실</b> ${esc(externalFacts)}</p>`;
}
...
function issueCard(issue, index, facts, evidence, gaps, contractFindings = [], readable, readableSource) {
  ...
  const rules = elements.map(e => {
    ...
    const description = isSourcePlaceholder(e.text || '') ? '출처가 확인되지 않아 적용할 법률 기준을 제시할 수 없습니다.' : readable(e.text || e.id);
    return `<div class="rv-rule"><strong>${esc(description)}</strong>${e.isException ? ' <span class="rv-tag">예외</span>' : e.mandatory === false ? ' <span class="rv-tag">택일</span>' : ''}${e.fallback ? ' <span class="rv-tag">요건 미검증</span>' : ''}<div class="rv-sources">${sources(a?.authorityEvidenceIds || [], evidence, readableSource)}</div></div>`;
  });
  const applications = assessments.map(a => {
    const e = elements.find(x => x.id === a.elementId);
    const elementDescription = isSourcePlaceholder(e?.text || '') ? '출처를 확인한 뒤 적용할 법률 기준을 정해야 합니다.' : readable(e?.text || '판단 요건');
    ...
    return `<div class="rv-application ${(c.decidingElementIds || []).includes(a.elementId) ? 'rv-deciding' : ''}"><div class="rv-application-head"><strong>${esc(elementDescription)}</strong><span class="rv-status">${esc(statuses)}</span></div>
      ...
      <p><b>사실에 대입한 판단</b> ${esc(readable(a.analysis || '판단 내용 없음'))}</p>
  ...
  return `<article class="opinion-item-card rv-issue">
    ...
    <div class="rv-flow">
      ${step('I', '무엇이 문제인가', `<p class="rv-question">${esc(readable(issue.question))}</p>${list(relatedFacts.map(f => esc(readable(f.text))), documentFacts.length ? '계약 원문은 위에 표시했습니다.' : '이 쟁점에 연결된 사실이 없습니다.')}`)}
      ${step('R', '어떤 법률 기준을 적용하나', rules.length ? rules.join('') : '<p class="rv-empty">이 쟁점에 적용할 법률 기준을 확인하지 못했습니다. 공식 법령을 확인해야 합니다.</p>')}
      ${step('A', '기준을 사실에 대입하면', applications.length ? applications.join('') : '<p class="rv-empty">법률 기준을 실제 사실에 대입한 판단이 아직 없습니다.</p>')}
      ${step('C', '쟁점별 결론과 남은 확인 사항', conclusionBody(issue, contract, label, tone, readable))}
    </div>
    ${issue.counter?.position ? `<div class="rv-followup"><b>추가 검토 의견</b><p><b>제기된 의견</b> ${esc(readable(issue.counter.position))}</p><p>${issue.counter.response ? `<b>검토 내용</b> ${esc(readable(issue.counter.response))}` : '이 의견에 대한 검토 내용이 작성되지 않았습니다.'}</p></div>` : ''}
...
function synthesis(reasoning, review, readable) {
  ...
  const rows = (reasoning.issues || []).map((issue, index) => {
    ...
    const result = issueConclusion(issue, finding, readable);
    const explanation = finding ? result.legal : result.summary;
    const reason = result.reasons.join(' / ') || (finding ? explanation : '판단 이유가 기록되지 않았습니다.');
    return `<tr><td>${index + 1}</td><td>${esc(readable(issue.question))}</td><td><span class="rv-verdict rv-${tone}">${esc(label)}</span>${finding ? `<p>${esc(explanation)}</p>` : ''}</td><td>${esc(reason)}</td></tr>`;
  }).join('');
...
export function renderReasoningOpinion(reasoning, review = {}, { report = false } = {}) {
  ...
  return `<div class="rv-report"><div class="rv-intro"><div><span class="rv-eyebrow">쟁점별 검토</span><h3>계약 문구의 위험과 법적 판단</h3><p>각 쟁점을 따로 살펴봅니다. 위험도가 높다는 표시는 계약 문구에 대한 평가이며, 해당 조항이 무효라는 확정 판정은 아닙니다. 법적 판단이 유보됐다면 이유와 필요한 확인 사항을 결론에 적었습니다.</p></div><span class="rv-count">${issues.length}개 쟁점</span></div>
