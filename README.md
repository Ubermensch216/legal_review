...
6. [20종 실무 벤치마크 및 테스트 결과](#20종-실무-벤치마크-및-테스트-결과)
...
### 탭 1: [1. 검토 초안]
- **조문 실존성 검증 뱃지**: 국가법령정보 DB와 대조하여 99% 확정 신뢰도 및 자동 보정 내역 표시
...
2. **하이브리드 검색 & 시맨틱 Re-ranking (Retrieval Engine)**
   ...
   - 3단계 체계적 연쇄 검색 (`cascadingRetriever.js`)
...
3. **20년 베테랑 IRAC 추론 & 환각 방지 (Reasoning & Quality)**
   ...
   - `factualityVerifier`: LLM 인용 조문 실존성 전수 교차 검증 및 오인용 자동 교정
4. **UI 렌더링 & 공문서 정형화 출력**
   - 램프 오프/온 뱃지 라이프사이클 관리, 절제된 Material Symbols 아이콘 적용
   - HWPX/PDF/DOCX 공문서 출력 시 신구 조문 대비표 및 조문 검증 필증 자동 내장
5. **2단계 고속 캐시 최적화**
...
- Node.js 18.0.0 이상
- npm 9.0.0 이상
- (선택) 로컬 LLM 구동 시: [Ollama](https://ollama.ai) (`gemma4:e2b` 등)
...
## 20종 실무 벤치마크 및 테스트 결과

실제 공공기관 및 기업 실무에서 발생하는 **20종 고난도 법률 사안**을 대상으로 종합 정량 평가를 실시하였습니다.

```bash
# 1. 17개 단위 테스트 전체 실행
npm test

# 2. 20종 실무 법률 검토 벤치마크 실행
...
| `lawHistory` | 법령 제정/개정 연혁 및 시행 타임라인 조회 |
| `timeTravel` | 특정 시점 기준 유효 법령 상태 재현 |
| `annexes` | 법령 별표 및 서식 조회 |
| `verifyCitations` | 인용 조문 실존성 검증 |
| `toolRunner` | 다중 도구 일괄 실행 및 파이프라인 연계 |
| `toolRegistry` | 19대 도구 카탈로그 메타데이터 제공 |
