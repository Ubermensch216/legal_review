...
**현재 상태 (2026-09-23):** 기본 검토는 단일 호출 경로입니다. 단계형 검토(`REVIEW_PIPELINE=staged`)는 시험 중이며, 5건 진단 측정에서 쟁점 생략과 인용·속도 문제가 확인되어 기본값으로 전환하지 않았습니다. [현재 구현·검증 상태](docs/current-status.md)와 [단계형 비교 기록](docs/audit/pipeline-comparison-2026-09-23.md)을 먼저 확인하세요.
...
## 목차
1. [주요 핵심 기능](#주요-핵심-기능)
2. [탭 메뉴 구조 및 작동 메커니즘](#탭-메뉴-구조-및-작동-메커니즘)
...
- **원클릭 복원**: 우측 검토 이력 드로어에서 **[불러오기]** 버튼으로 검토 화면을 복원합니다.
...
### 탭 5: 외부 전문가 질의
- 미해결 질문을 비식별 질의서로 만들고, 사용자가 가져온 외부 AI·사람 전문가 답변을 질문별로 반입·승인합니다.
- 단계형 검토에서는 공백이 쟁점·요건에 연결되며, 공식 근거 지문이 같으면 연결 쟁점을 부분 재검토할 수 있습니다. 사실 확인 공백은 사용자에게 남깁니다.
...
3. **LLM 검토와 인용 확인 (Reasoning & Quality)**
   - 기본 단일 호출 검토 및 시험용 단계형 쟁점·요건 검토. 단계형 결과는 검토 초안 안의 구조화된 표로 표시
...
5. **2단계 고속 캐시 최적화**
   - 인메모리 L1 + SQLite L2 (WAL 모드) 캐시로 반복 조회를 재사용. 실제 응답 시간은 입력·모델·공식 API 상태에 따라 달라짐
...
기본 검토는 단일 호출입니다. 로컬 Ollama에서 단계형 경로를 시험할 때만 `.env`에 `REVIEW_PIPELINE=staged`를 설정합니다. 단계형은 현재 검증 중이며 기본 운영 경로로 권장하지 않습니다.
...
공식 API 인증과 로컬 Ollama가 있는 환경에서 **5개 샘플을 단일 호출·단계형으로 각 1회 진단 측정**했습니다. 단계형은 파싱 5/5건, 규칙 기반 폴백 0건이지만 02·05의 모든 쟁점이 생략됐고, 04의 인용 존재율은 82%였습니다. 사례 01의 모델 처리 시간은 단일 호출보다 27% 길었습니다. 기준선과 단계형 사이 코드 변경이 있어 동조건 품질 비교나 제품 성능 보증으로 사용할 수 없습니다. 수치와 방법은 [비교 기록](docs/audit/pipeline-comparison-2026-09-23.md)에 있습니다.
