...
- **사전 컨설팅감사 의견**: 적극행정 사전 컨설팅감사 신청에 대하여 대립 견해(갑설·을설)의 타당성을 비교하고 수용/반려 처리 의견을 제시 (자치법규·행정규칙 조문 근거 포함)

### 2. 검토 기준 시점 지정 (과거 법령 검토)

검토 요청에 **검토 기준 시점**을 지정하면, 그 날짜에 시행 중이던 법령 버전으로 검토합니다. 사건 발생일이나 계약 체결일이 현재와 다를 때 현행 조문으로 판단하는 것을 막기 위한 기능입니다.

- 기준 법령, 문서가 인용한 다른 법령, 그리고 법–시행령–시행규칙 연쇄 체계를 **모두 같은 시점 버전으로 맞춥니다.** 한 단계라도 그 시점 버전을 확인하지 못하면 현행 본문으로 대체하지 않고 제외한 뒤 제한 사항으로 표시합니다.
- 기준일 이후에 시행된 조문은 근거에서 제외하며, 인용 실존성 검증의 기준일도 그 시점으로 바뀝니다.
- 검토 화면과 HWPX/PDF/DOCX 보고서에 시점 검토라는 사실과 **부칙·경과조치는 별도 검토가 필요하다는 제한**이 함께 표시됩니다.
- 미지정 시에는 종전과 같이 현행 법령을 기준으로 검토합니다. 형식이 잘못된 기준일은 조용히 오늘 날짜로 대체하지 않고 요청을 거절합니다.

### 3. 브라우저 IndexedDB 영구 검토 이력 관리
...
### 4. 멀티포맷 첨부문서 정밀 파싱 및 공공서식 보고서 생성
...
2. **하이브리드 검색 & 시맨틱 Re-ranking (Retrieval Engine)**
   ...
   - 3단계 체계적 연쇄 검색 (`cascadingRetriever.js`) — 모법·시행령 조회 동작 확인됨 (시행규칙은 문언 인용이 있는 경우에만 연결)
...
> 공식 자료를 조회하지 않고 `UNAVAILABLE` 상태와 제한 사항을 반환합니다.
> 샘플은 `LAW_DEMO_MODE=true`로 명시한 경우에만 제공하며, 공식 근거로 검증하지 않습니다.
...
# .env 파일에 LAW_OC=본인계정ID 및 사용할 LLM 설정을 입력합니다.
...
| `lawHistory` | 법령 버전 목록 조회 — `target=eflaw`, `nw`와 `LID`를 사용해 동일 법령의 버전을 수집합니다. 실제 API 계약 검증은 별도 통합 테스트가 필요합니다. |
| `timeTravel` | 시행일 기준 버전 선택 및 본문 조회 — `MST`·`efYd`를 사용하며 요청 시점 이후 시행 버전을 제외합니다. 부칙의 개별 경과조치까지 자동 판정하지 않습니다. |
...
2026-09-21 수정 및 잔여 검증 범위는 [구현 검증 기록](docs/audit/implementation-2026-09-21.md)을 참고하십시오.
