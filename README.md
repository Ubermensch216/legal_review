...
- **[1단계: 검토 초안 (Review Draft)]**
  ...
  - 쟁점 및 법적 리스크 분석표 (HIGH / MEDIUM / LOW 등급별 제재 및 과태료 위험 분석 테이블)
  - 심층 법률 검토의견 (본론)
  - 보완 권고사항 및 조치 계획 체크리스트
  - 공식 법률검토의견서 (공문서 표준 서식 뷰)
...
- **[3단계: 개정 & 영향 분석 (Revisions & Impacts)]**
  - 첨부문서 내 조문 인용 충돌 및 적합성 분석표
  - 삭제/폐지된 과거 구(舊) 조문 인용 시 즉시 경고 알림

### 3. 검토 이력(History) 자동 저장 및 원클릭 복원
- 검토를 수행할 때마다 SQLite DB에 질의, 첨부문서, 주요 법령, 결과 요약 및 전체 분석 데이터가 자동 영구 보관됩니다.
- 상단 헤더의 **[검토 이력]** 버튼을 통해 과거 검토 목록을 타임라인별로 조회하고, **[불러오기]** 버튼을 클릭하여 당시의 3단계 탭 화면을 즉시 원클릭으로 복원할 수 있습니다.
